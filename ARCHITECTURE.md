...
| **流水线超时** | `asyncio.wait_for(180s)` → `JobStatus.TIMEOUT` 写入数据库 |
...
2. **LLM 调用** — gpt-4o-mini 约 2-5 秒；通过仓库内容哈希缓存
